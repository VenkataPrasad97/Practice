...
for i in range(2):
    a=[]
    for i in range(nvar):
        a.append(round(random.uniform(100,200)))
...
def connect_1(a_index,d_index,betta,varmin,varmax):
    ...
    b=np.sin(betta)*a
    y=[]
    x1=persons_df['Position'].iloc[a_index]
    x2=persons_df['Position'].iloc[d_index]
    for i in x1:
        for j in x2:
            c=i*((1-b)+(i+j)/2)*b
            z=max(c,varmin)
...
for i in range(2):
    for j in range(2):
        if i==j:
            continue
        if persons_df['Cost1'].iloc[i]<=persons_df['Cost2'].iloc[j]:
            attacker_index=i
        else:
            attacker_index=j
if attacker_index==0:
...
for it in range(2,maxit+1):
    new_defender=persons_df['Position'].iloc[defender_index]
    for i in range(gamma):
        temp=np.random.randint(0,nvar)
        new_defender[temp]=persons_df['Position'].iloc[attacker_index][temp]
    ...
    if  calc_objective1(new_defender)<=calc_objective1(persons_df['Position'].iloc[defender_index]) or calc_objective2(new_defender)<=calc_objective2(persons_df['Position'].iloc[defender_index]):
        persons_df['Position'].iloc[defender_index]=new_defender
    # Spotting a attack
    ...
    if calc_objective1(new_defender)<=calc_objective1(persons_df['Position'].iloc[defender_index]) or calc_objective2(new_defender) <= calc_objective2(persons_df['Position'].iloc[defender_index]):
        if calc_objective1(new_defender)<=calc_objective1(persons_df['Position'].iloc[attacker_index]) and calc_objective2(new_defender) <= calc_objective2(persons_df['Position'].iloc[attacker_index]):
            persons_df['Position'].iloc[attacker_index]=new_defender
        else:
            persons_df['Position'].iloc[defender_index]=new_defender
    # elif <= or <=:
    #     ParetoCost(it)=new_defender #check can be combined with above
    elif calc_objective1(new_defender)>calc_objective1(persons_df['Position'].iloc[defender_index]) or calc_objective2(new_defender) > calc_objective2(persons_df['Position'].iloc[defender_index]):
        if calc_objective1(persons_df['Position'].iloc[defender_index])<=calc_objective1(persons_df['Position'].iloc[attacker_index]) and calc_objective2(persons_df['Position'].iloc[defender_index]) <= calc_objective2(persons_df['Position'].iloc[attacker_index]):
            persons_df['Position'].iloc[attacker_index]=persons_df['Position'].iloc[defender_index]
    else:
        print("ERROR")
    paretoposition.append(persons_df['Position'].iloc[attacker_index])
    paretocost=calc_objective1(persons_df['Position'].iloc[attacker_index])
